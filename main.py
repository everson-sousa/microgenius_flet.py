...
# # --- INICIA O JOGO ---
# # Use ft.app(target=main) para rodar como app desktop
# # Use ft.app(target=main, view=ft.WEB_BROWSER) para rodar no navegador
...
class JogoGenius:
    def __init__(self):
        self.sequencia = []
        self.resposta_jogador = []
        self.estado = "INICIO" # "INICIO", "MOSTRANDO", "JOGANDO", "FIM_DE_JOGO"
        self.recorde = self.carregar_recorde()
    ...
    def salvar_recorde(self):
        # A pontuação é o número de acertos, que é o nível anterior ao erro.
        pontos = len(self.sequencia) - 1
        if pontos > self.recorde:
            self.recorde = pontos
            with open(ARQUIVO_RECORDE, "w") as f: f.write(str(self.recorde))
    ...
    def iniciar_novo_jogo(self):
        self.sequencia = []
        self.resposta_jogador = []
        self.estado = "MOSTRANDO" # Já vai direto para mostrar o primeiro nível
        self.proximo_nivel()
    ...
    def registrar_jogada(self, cor_id):
        ...
        if len(self.resposta_jogador) == len(self.sequencia):
            time.sleep(0.5) # Pequena pausa antes de mostrar a próxima sequência
            self.proximo_nivel()
...
class AppGenius(ft.Column):
    ...
    def ao_clicar(self, e: ft.TapEvent):
        ...
        if estado_anterior == "INICIO" and cor_clicada == "centro":
            ...
            threading.Thread(target=self.processar_jogada_thread, args=(cor_clicada,)).start()
        elif estado_anterior == "FIM_DE_JOGO" and cor_clicada == "centro":
            self.jogo.iniciar_novo_jogo()
            self.atualizar_visual()

    def processar_jogada_thread(self, cor_id):
        self.piscar_cor(cor_id, 0.2)
        self.jogo.registrar_jogada(cor_id)
        self.atualizar_visual()

    def atualizar_visual(self):
        pontos_atuais = len(self.jogo.sequencia)
        if self.jogo.estado == "INICIO":
            self.texto_pontos.value = f"Recorde: {self.jogo.recorde}"
            self.texto_status.value = "Clique em Micro Genius para iniciar"
        elif self.jogo.estado == "JOGANDO":
            self.texto_pontos.value = f"Pontos: {pontos_atuais - 1}"
            self.texto_status.value = "Sua vez!"
        elif self.jogo.estado == "FIM_DE_JOGO":
            pontos_finais = len(self.jogo.sequencia) - 1
            self.texto_pontos.value = f"Recorde: {self.jogo.recorde}"
            self.texto_status.value = f"Fim de Jogo! Pontos: {pontos_finais}. Clique para recomeçar."
        
        self.update()
        
        if self.jogo.estado == "MOSTRANDO":
            threading.Thread(target=self.mostrar_sequencia_thread).start()

    def mostrar_sequencia_thread(self):
        self.tabuleiro_interativo.disabled = True
        self.texto_status.value = "Observe..."
        pontos = len(self.jogo.sequencia)
        self.texto_pontos.value = f"Pontos: {pontos}"
        self.update()
        time.sleep(1)
